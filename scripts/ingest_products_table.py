...
class IngestProductsTable:
  ...
  def add_text_embedding(self, product: Product):
  ...
  def add_image_embedding(self, product: Product):
    logging.info(f"Adding image embedding to product {product.code}...")
    image_url = product.image_url
    embedding = self.embeddings_service.embed_image(image_url)
  ...
  def ingest_products_table(self, directory: str):
    ...
    for product in products:
      product = self.add_text_embedding(product)
      product = self.add_image_embedding(product)
